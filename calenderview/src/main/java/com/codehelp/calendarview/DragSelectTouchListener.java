...
public class DragSelectTouchListener implements RecyclerView.OnItemTouchListener {
     ...
     */
    DragSelectTouchListener withSelectListener(OnDragSelectListener selectListener) {
     ...
     */
    void startDragSelection(int position) {
        setIsActive(true);
        mStart = position;
        mEnd = position;
        mLastStart = position;
    ...
    public boolean onInterceptTouchEvent(@NonNull RecyclerView rv, @NonNull MotionEvent e) {
        if (!mIsActive || Objects.requireNonNull(rv.getAdapter()).getItemCount() == 0)
            return false;
